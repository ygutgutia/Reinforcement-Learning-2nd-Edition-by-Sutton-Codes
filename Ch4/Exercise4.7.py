...
class jcp:
    def __init__(self, max_cars, disc_rate, credit_reward, moving_reward, second_parking_lot_reward):
        self.max_cars = max_cars
        self.disc_rate = disc_rate
        self.credit_reward = credit_reward
        self.moving_reward = moving_reward
        self.second_parking_lot_reward = second_parking_lot_reward
    ...
    def expected_reward(self, state, action):
        ...
        if action <= 0:
            ...
            reward = reward + self.moving_reward * (action - 1) # one car is moved by one of Jack's employees for free
        # adding reward for second parking lot (which is also negative)
        if new_state[0] > 10:
            reward = reward + self.second_parking_lot_reward
        if new_state[1] > 10:
            reward = reward + self.second_parking_lot_reward
